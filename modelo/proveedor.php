	...
	class Proveedor extends Conectar{
		...
		*/
		...
		public function cargarRegistro(){
            ...
            echo "<div class='row'>";
            echo 	"<div class='col-md-4'>";
            echo		"<label>Continente</label>";
            			$objContinente = new ubicacion();
            			
            			echo "<select name='continente' id='Continente'>";
            			echo "<option value=''>Seleccione...</option>";
            			while($cnt=mysql_fetch_array($objContinente->Continente())){
            				echo "<option value=''>Seleccione...</option>";
            			}
            			echo "</select>";
            ...
            echo 	"<div class='col-md-3'>
            			<label>Ciudad</label>
            			<input type='text' class='form form-control' id='ciudad' value=''>
            		</div>";
            echo 	"<div class='col-md-3'>
            			<label>Correo</label>
		...
		function cargarLista(){
			$sqlEmp=mysql_query("SELECT * FROM proveedor ORDER BY nombre");
		...
		function cargarProveedor($idProveedor){		
			$sqlBuscar=mysql_query("SELECT * FROM proveedor WHERE `idProveedor` = '$idProveedor';");
		...
		function agregar($idProveedor,$nombre,$direccion,$telefono,$ciudad,$correo){
			$sqlAdd = mysql_query("INSERT INTO proveedor (`idProveedor`,`nombre`, `Dir`, `TEL`,`CIUDAD`, `correo`) VALUES ('$idProveedor','$nombre','$direccion','$telefono','$ciudad','$correo');");
				$this->cargarLista();
		}
		...
		function cargarPagos($idProveedor){		
			$sqlBuscar=mysql_query("SELECT * FROM proveedor WHERE `ID_EMPLEADO` = '$idProveedor';");
			//echo "<script>alertify.success('Llego el dato del proveedor al modelo id: $idProveedor; hasta aquí no sé que pasa si sigue mostrando el listado');</script>";
			while($pro=mysql_fetch_array($sqlBuscar)){	
		...
		function cargarListaPagos($idProveedor){
			...
			$sqlEmp=mysql_query("SELECT fc.factura,fc.factRegistro,fc.TOTAL,SUM(ap.Valor_Abono)AS 'Total Abonos al crédito',(fc.TOTAL-SUM(ap.Valor_Abono)) AS 'Saldo',fc.tipo FROM facturasc fc LEFT JOIN abonosproveedor ap ON ap.idCompra=fc.FACTURA WHERE fc.idProveedor='$idProveedor' GROUP BY fc.FACTURA;");
			echo "<div class='panel panel-default>";
			echo 	"<div class='panel-heading'>";
			$sqlProveedor=mysql_query("SELECT nombre FROM proveedor WHERE idProveedor='$idProveedor'");
